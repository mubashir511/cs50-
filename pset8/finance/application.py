...
def index():
    ...
    return render_template("index.html", search_buy=search_buy, search_len=search_len, search_users=search_users)
    ...
    if request.method == "POST":
        ...
        if not chk_empty:
            ...
            if not search:
                ...
                db.execute("UPDATE buy SET shares=:shares, price=:price, total=:total WHERE id=:id AND symbol=:symbol", shares = (int(request.form.get("shares")) + int(search[0]["shares"])),
                            price = quote['price'], total = quote['price']*float(request.form.get("shares")), id = session["user_id"], symbol=request.form.get("symbol"))
...
def sell():
    ...
    if request.method == "POST":
        ...
        # search buy table
        search_buy = db.execute("SELECT symbol FROM buy WHERE id=:id", id=session["user_id"])
        # find the max length of the search data
        search_len = len(search_buy)
        print(search_buy)
