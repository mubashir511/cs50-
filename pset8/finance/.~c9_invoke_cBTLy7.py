...
def buy():
    ...
    if request.method == "POST":
        ...
        # Input buying data into buy table
        db.execute("UPDATE buy SET username=:username, companyName=:companyName, symbol=:symbol, shares=:shares, price=:price, total=:total WHERE id=:id",
...
def register():
    ...
    if request.method == "POST":
        ...
        id_count += 1
        result = db.execute("INSERT INTO users (id, username,hash) VALUES(:id, :username,:hash)", id = id_count, username = request.form.get("username"), hash = hashPass)
